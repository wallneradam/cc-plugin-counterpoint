---
name: counterpoint-review-loop
description: >
  Autonomous review-and-fix loop on a single persistent Codex thread via the counterpoint
  plugin. Invoke when the user explicitly asks for it — via `/counterpoint:review_loop` or
  phrases like "run the review loop", "fix the review findings automatically", "review until
  clean". Reviews the local git state, verifies every finding before fixing, fixes the real
  ones, reports back to Codex, and repeats until Codex approves or the loop converges.
  Do NOT invoke proactively.
---

# Counterpoint Review Loop

An autonomous loop around the counterpoint `review` mode: review → verify each finding → fix the real ones → report back to Codex → re-review, until clean or a safety cap.
...
**The orchestrator** (the agent that invoked this skill):
...
EXTRA REVIEW ARGS: [--scope/--base/--path/--effort flags passed to the skill, or "none"]
